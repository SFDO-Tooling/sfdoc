...
class HTML:
    ...
    def __init__(self, htmlpath, rootpath):
        ...
        for attr, tag_name, optional in (
            ...
            (
                ...
                "ProductMapUUID",
                True,
            ),  # Should be required but will break a lot of tests,
            ...
            if not tag:
                ...
                raise HtmlError(
                    "Meta tag name={} has no content".format(
                        tag_name,
                    )
                )
        ...
        if not body_tag:
            raise HtmlError(
                "Body tag <div class={} ...> not found".format(
    ...
    def same_as_record(self, record, logger):
        ...
        def compare(item1, item2, name):
            if not item1 and not item2:
                ...
                if item1 == item2:
                    ...
                    item1_summary = (
                        (item1[:20] + "...") if item1 and len(item1) > 75 else item1
                    )
                    item2_summary = (
                        (item2[:20] + "...") if item2 and len(item2) > 75 else item2
                    )
        ...
        differences = []
        compare(self.author, record[settings.SALESFORCE_ARTICLE_AUTHOR_FIELD], "author")
        compare(
            self.author_override,
            record[settings.SALESFORCE_ARTICLE_AUTHOR_OVERRIDE_FIELD],
            "author_override",
        )
        compare(self.is_visible_in_csp, record["IsVisibleInCsp"], "is_visible_in_csp")
        compare(self.is_visible_in_pkb, record["IsVisibleInPkb"], "is_visible_in_pkb")
        compare(self.is_visible_in_prm, record["IsVisibleInPrm"], "is_visible_in_prm")
        compare(self.title, record["Title"], "title")
        compare(self.summary, record["Summary"], "summary")
        compare(
                        ...
                        for attr in child.attrs:
                            ...
                            if attr in ("href", "src"):
                                if not is_url_whitelisted(child[attr]):
                                    problems.append(
                                        "URL {} not whitelisted".format(child[attr])
                                    )
                        scrub_tree(child)
