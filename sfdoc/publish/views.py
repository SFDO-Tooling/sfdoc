...
def review(request, pk):
    ...
    context = {
        ...
        **common_context,
        "form": form,
        "articles_new": get_articles_for_view(
            bundle.articles.filter(status=Article.STATUS_NEW).order_by("url_name")
        ),
        "articles_changed": get_articles_for_view(
            bundle.articles.filter(status=Article.STATUS_CHANGED).order_by("url_name")
        ),
        "articles_deleted": get_articles_for_view(
            bundle.articles.filter(status=Article.STATUS_DELETED).order_by("url_name")
        ),
        "images_new": bundle.images.filter(status=Image.STATUS_NEW).order_by(
            "filename"
        ),
        "images_changed": bundle.images.filter(status=Image.STATUS_CHANGED).order_by(
            "filename"
        ),
        "images_deleted": bundle.images.filter(status=Image.STATUS_DELETED).order_by(
            "filename"
        ),
    }
