...
class TestTasks(TestCase):
    def test_process_bundle_queues_simple_case(self):
        with mock.patch("sfdoc.publish.tasks.process_bundle.delay") as mock_method:
            ...
            mock_method.assert_has_calls(
                [call(bundle1.pk), call(bundle2.pk), call(bundle3.pk)], any_order=True
            )
    ...
    def test_process_bundle_queues_complex_case(self):
        with mock.patch("sfdoc.publish.tasks.process_bundle.delay") as mock_method:
            ...
            bundle2 = BundleFactory(status=Bundle.STATUS_QUEUED)
            bundle3 = BundleFactory(status=Bundle.STATUS_QUEUED)
            bundle4 = BundleFactory(
                status=Bundle.STATUS_QUEUED,
                easydita_resource_id=bundle1.easydita_resource_id,
            )
            bundle5 = BundleFactory(
                status=Bundle.STATUS_QUEUED,
                easydita_resource_id=bundle2.easydita_resource_id,
            )
            bundle6 = BundleFactory(
                status=Bundle.STATUS_QUEUED,
                easydita_resource_id=bundle3.easydita_resource_id,
            )
