...
def mock_create_article(instance_url, kav_id):
    url = "{}/services/data/v{}/sobjects/{}/".format(
        instance_url, settings.SALESFORCE_API_VERSION, settings.SALESFORCE_ARTICLE_TYPE
    )
...
def mock_create_draft(instance_url, ka_id, kav_id):
    url = (
        "{}/services/data/v{}" "/knowledgeManagement/articleVersions/masterVersions"
    ).format(instance_url, settings.SALESFORCE_API_VERSION)
    responses.add("POST", url=url, status=HTTPStatus.CREATED, json={"id": str(kav_id)})
...
def mock_bundle_download(url, articles):
    ...
    with ZipFile(zip_buff, mode="w") as f_zip:
        for a in articles:
            f_zip.writestr(
                a["filename"],
                create_test_html(a["url_name"], a["title"], a["summary"], a["body"]),
            )
    responses.add(
        "GET", url=url, body=zip_buff.getvalue(), content_type="application/zip"
    )
...
def mock_publish_draft(instance_url, kav_id):
    url = (
        ...
        + "/masterVersions/{}"
    ).format(instance_url, settings.SALESFORCE_API_VERSION, kav_id)
    responses.add("PATCH", url=url, status=HTTPStatus.NO_CONTENT)


def mock_query(instance_url, url_name, publish_status, fields=None, return_val=None):
    ...
    url = "{}/services/data/v{}/query/?{}".format(
        instance_url, settings.SALESFORCE_API_VERSION, query_s
    )
    responses.add("GET", url=url, match_querystring=True, json=return_val)
